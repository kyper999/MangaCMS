...
class DbBase(MangaCMS.lib.LogBase.LoggerMixin, TransactionMixin, metaclass=abc.ABCMeta):
	...
	def __del__(self):
		if hasattr(self, 'db_connection_dict'):
			for conn in self.db_connection_dict:
				MangaCMS.lib.dbPool.pool.putconn(conn)
	...
	def __thread_cursor(self):
		...
		self.db_connection_dict[tid] = MangaCMS.lib.dbPool.pool.getconn()
