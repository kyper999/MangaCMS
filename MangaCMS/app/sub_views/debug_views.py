...
cpool = psycopg2.pool.ThreadedConnectionPool(
							minconn  = 2,
							maxconn  = 5,
							dbname   = settings.DEDUPER_PSQL_DB_NAME,
							host     = settings.DEDUPER_PSQL_IP,
							password = settings.DEDUPER_PSQL_PASS,
							user     = settings.DEDUPER_PSQL_USER,
	)
...
def get_deduper_resource(rowid):
	...
	if fspath and not intpath:
		with open(fspath, "rb") as itemFileHandle:
			...
			response.headers['Content-Type']        = reader_views.guessItemMimeType(fspath)
		...
		response = make_response(itemFileHandle.read())
		response.headers['Content-Type']        = reader_views.guessItemMimeType(intpath)
